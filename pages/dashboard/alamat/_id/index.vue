...
export default {
    ...
    async mounted() {
        var response = []
        await this.$axios.get(process.env.baseUrl + 'getaddressbyid/' + this.$route.params.id + '.json')
            .then(res => {
                response = res
                this.id = res.data.list.id
                this.id_user = res.data.list.id
                this.nama = res.data.list.nama
                this.company = res.data.list.company
                this.no_hp = res.data.list.no_hp
                this.email = res.data.list.email
                this.catatan = res.data.list.catatan
                this.tipe_user = res.data.list.tipe_user
                this.id_kota = res.data.list.id_kota
                this.id_negara = res.data.list.id_negara
                this.kode_pos = res.data.list.kode_pos
                this.alamat = res.data.list.alamat
                this.created_by = res.data.list.created_by
            }).catch(e => {
                content.error(e)
            })
        $(document).ready(function () {
            $("#kode_kota").val(response.data.list.kota.id)
            $("#kota_text").val(response.data.list.kota.nama)
            $("#kode_negara").val(response.data.list.negara.id)
            $("#negara_text").val(response.data.list.negara.nama)
            $("#kode_kota").select2({
                placeholder: "No kota",
                initSelection: function (element, callback) {
                    callback({
                        id: response.data.list.id_kota,
                        text: response.data.list.kota.nama
                    });
                },
            ...
            $("#kode_negara").select2({
                ...
                initSelection: function (element, callback) {
                    callback({
                        id: response.data.list.id_negara,
                        text: response.data.list.negara.nama
                    });
                },
                minimumInputLength: 1,
                ajax: {
    ...
    methods: {
        ...
        async updateaddress() {
            ...
            this.$store.dispatch('updateAddress', bodyFormData).then(() => {
